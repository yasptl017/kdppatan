...
<?php
include 'dptname.php';
$page_title = "Results - " . $DEPARTMENT_NAME . " - K.D. Polytechnic";
?>
...
<body>
        ...
        <div class="container">
            <?php if ($results_result && $results_result->num_rows > 0): ?>
                ...
                <div class="row g-4" id="resultsGrid">
                    <?php while ($result = $results_result->fetch_assoc()): ?>
                        <div class="col-lg-4 col-md-6 results-item" data-title="<?php echo strtolower(htmlspecialchars($result['title'])); ?>">
                            <div class="document-card">
                                <div class="document-icon">
                                    <i class="fas fa-file-pdf"></i>
                                </div>
                                <div class="document-content flex-grow-1">
                                    <h5 class="document-title" title="<?php echo htmlspecialchars($result['title']); ?>">
                                        <?php echo htmlspecialchars($result['title']); ?>
                                    </h5>
                                </div>
                                <a href="../Admin/<?php echo htmlspecialchars($result['file_path']); ?>"
            ...
            <?php else: ?>
                <div class="no-data-message">
                    ...
                    <p>Results for this department will be available soon.</p>
                </div>
            <?php endif; ?>
        </div>
    </section>
    ...
    <script>
        ...
        if (resultsSearch) {
            resultsSearch.addEventListener('keyup', function() {
                ...
                if (visibleItems.length === 0) {
                    if (!noResultsMsg) {
                        ...
                        msg.innerHTML = '<i class="fas fa-search"></i><h4>No Results Found</h4><p>Try a different search term.</p>';
