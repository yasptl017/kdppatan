...
if (isset($_GET['move'], $_GET['id'])) {
    ...
    $current = $conn->query(
        "SELECT id, display_order FROM hostel WHERE id=$id"
    )->fetch_assoc();
    ...
    if ($current) {
        ...
        if ($direction === 'up') {
            $swap = $conn->query(
                "SELECT id, display_order FROM hostel
        ...
        if ($swap) {
            $conn->query(
                "UPDATE hostel SET display_order={$swap['display_order']}
                 WHERE id={$current['id']}"
            );
            $conn->query(
                "UPDATE hostel SET display_order={$current['display_order']}
                 WHERE id={$swap['id']}"
            );
        }
    }
...
if (isset($_POST['save_hostel'])) {
    ...
    // GET OLD PHOTOS IF UPDATING
    $oldPhotos = [];
    if ($id > 0) {
        $q = $conn->query("SELECT photos FROM hostel WHERE id=$id LIMIT 1");
        if ($q && $q->num_rows > 0) {
        ...
        foreach ($_FILES['photos']['name'] as $index => $fileName) {
            ...
            if ($_FILES['photos']['error'][$index] == 0) {
                ...
                if (in_array($ext, $allowed)) {
                    ...
                    $newName = "hostel_" . time() . "_" . rand(1000,9999) . "." . $ext;
    ...
    $photosJSON = json_encode($photoArray, JSON_UNESCAPED_SLASHES);
    $photosJSON = $conn->real_escape_string($photosJSON);

    // Insert / Update Query
    if ($id == 0) {
        $sql = "INSERT INTO hostel (title, display_order, photos, description)
                VALUES ('$title', $display_order, '$photosJSON', '$description')";
        $conn->query($sql);
        $message = "Hostel information added successfully!";
        $messageType = "success";
    } else {
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM hostel WHERE id=$id");
    $message = "Hostel record deleted!";
    $messageType = "success";
}

$records = $conn->query("SELECT * FROM hostel ORDER BY display_order ASC, id DESC");
?>
...
<main class="main-content" id="mainContent">
    ...
    <h2 class="h4 mb-4"><i class="fas fa-home me-2"></i>Manage Hostel</h2>
    ...
    <div class="form-card">
        <table id="hostelTable" class="table table-bordered table-striped align-middle">
            <thead>
            ...
            <?php $i=1; while($row=$records->fetch_assoc()): ?>
                <tr>
                    ...
                    <td class="text-center">
                        <?php 
                        $pics = json_decode($row['photos'], true);
                        if (!empty($pics)): ?>
                            <img src="<?php echo htmlspecialchars($pics[0]); ?>" height="60" class="rounded" style="object-fit:cover;">
                        <?php else: ?>
                    ...
                    <td>
                        <div class="action-buttons">
                            ...
                            <button class="btn btn-warning btn-sm"
                                    onclick='editHostel(<?php echo json_encode($row, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>)'
                                    data-bs-toggle="modal"
                                    data-bs-target="#hostelModal"
                                    title="Edit">
                            ...
                            <a href="?delete=<?php echo $row['id']; ?>"
                               onclick="return confirm('Delete this hostel record and all its photos?');"
        ...
        <div class="modal-content">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-footer">
                    <button type="submit" name="save_hostel" class="btn btn-success">
...
$('#hostelModal').on('shown.bs.modal', function () {
    if (!$('#description').next('.note-editor').length) {
        initSummernote('#description', {height: 250, placeholder: "Enter hostel description..."});
    }
...
// Add Hostel
function addHostel() {
    document.getElementById("modalTitle").innerText = "Add Hostel";
...
// Edit Hostel
function editHostel(f) {
    document.getElementById("modalTitle").innerText = "Update Hostel";
    document.getElementById("hostel_id").value = f.id;
...
console.log('✓ Hostel Management page loaded');
</script>
