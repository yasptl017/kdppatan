...
$uploadDir = "uploads/dept_timetable/";
...
if (isset($_POST['save_timetable'])) {
    ...
    $oldFile = "";
    if ($id > 0) {
        $old = $conn->query("SELECT file_path FROM dept_timetable WHERE id=$id LIMIT 1");
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === 0) {
        ...
        if (in_array($ext, $allowed)) {
            $filePath = $uploadDir . "dept_timetable_" . time() . "_" . rand(1000, 9999) . "." . $ext;
    ...
    if (empty($message)) {
        if ($id === 0) {
            if (empty($filePath)) {
                ...
                $sql = "INSERT INTO dept_timetable (department, title, display_order, file_path)
...
if (isset($_GET['delete'])) {
    ...
    if ($conn->query("DELETE FROM dept_timetable WHERE id=$id")) {
...
if ($_SESSION['role'] == 'Admin') {
    $records = $conn->query("SELECT * FROM dept_timetable ORDER BY department ASC, display_order ASC, id DESC");
    ...
    $records = $conn->query("SELECT * FROM dept_timetable WHERE department='$userDept' ORDER BY display_order ASC, id DESC");
...
<main class="main-content" id="mainContent">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="h4 mb-0"><i class="fas fa-calendar-check me-2"></i>Manage Department Time Tables</h2>
        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#timetableModal" onclick="addTimetable()">
            <i class="fas fa-plus-circle"></i> Add Time Table
        </button>
    </div>
    ...
    <div class="form-card">
        <table id="timetableTable" class="table table-striped table-bordered table-hover align-middle">
                            ...
                            <td class="text-center">
                                <div class="action-buttons">
                                    <button class="btn btn-warning btn-sm" title="Edit"
                                            onclick='editTimetable(<?php echo json_encode($row, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>)'
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">Add Time Table</h5>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" id="title" class="form-control" placeholder="e.g., Semester 1 Timetable" required>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" name="save_timetable" class="btn btn-primary">Save</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    function addTimetable() {
        ...
        document.getElementById('modalTitle').textContent = 'Add Time Table';
    ...
    function editTimetable(data) {
        ...
        document.getElementById('modalTitle').textContent = 'Edit Time Table';
    ...
    document.getElementById('timetableModal').addEventListener('hidden.bs.modal', function () {
        document.getElementById('title').value = '';
        document.getElementById('display_order').value = '0';
        document.getElementById('file').value = '';
        document.getElementById('file').removeAttribute('required');
