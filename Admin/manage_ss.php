...
if (isset($_GET['move'], $_GET['id'])) {
    ...
    $current = $conn->query(
        "SELECT id, display_order FROM ss WHERE id=$id"
    )->fetch_assoc();
    ...
    if ($current) {
        ...
        if ($direction === 'up') {
            $swap = $conn->query(
                "SELECT id, display_order FROM ss
        ...
        if ($swap) {
            $conn->query(
                "UPDATE ss SET display_order={$swap['display_order']}
                 WHERE id={$current['id']}"
            );
            $conn->query(
                "UPDATE ss SET display_order={$current['display_order']}
                 WHERE id={$swap['id']}"
            );
        }
    }
...
/* ===================================
   ADD / UPDATE SS
   =================================== */
if (isset($_POST['save_ss'])) {

    $id = intval($_POST['ss_id']);
    ...
    $oldFile = "";
    if ($id > 0) {
        $q = $conn->query("SELECT file FROM ss WHERE id=$id LIMIT 1");
        if ($q->num_rows > 0) {
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
        ...
        if (in_array($ext, $allowed)) {
            $fileName = "ss_" . time() . "_" . rand(1000, 9999) . "." . $ext;
    ...
    if (empty($message)) {
        ...
        if ($id == 0) {
            $sql = "INSERT INTO ss (title, display_order, file, description)
                    VALUES ('$title', $display_order, '$filePath', '$description')";
            $conn->query($sql);
            $message = "Student Section record added successfully!";
        } else {
            $sql = "UPDATE ss SET 
                    title='$title',
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM ss WHERE id=$id");
    $message = "Student Section record deleted!";
    $messageType = "success";
}

$records = $conn->query("SELECT * FROM ss ORDER BY display_order ASC, id DESC");
?>
...
<h2 class="h4 mb-4"><i class="fas fa-clipboard-list me-2"></i>Manage Student Section Programs</h2>
...
<button class="btn btn-primary mb-3"
        ...
        data-bs-target="#ssModal"
        onclick="addSS()">
    <i class="fas fa-plus-circle"></i> Add Student Section Record
</button>
                ...
                <td>
                    <div class="action-buttons">
                        ...
                        <button class="btn btn-warning btn-sm"
                                onclick='editSS(<?php echo json_encode($row, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>)'
                                data-bs-toggle="modal"
                                data-bs-target="#ssModal">
                        ...
                        <a href="?delete=<?php echo $row['id']; ?>"
                           onclick="return confirm('Delete this Student Section record?');"
        ...
        <div class="modal-content">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="modalTitle">Add Student Section</h5>
                ...
                <div class="modal-footer">
                    <button type="submit" name="save_ss" class="btn btn-success">
...
$('#ssModal').on('hidden.bs.modal', function () {
    if ($('#description').next('.note-editor').length) {
...
// Add
function addSS() {
    document.getElementById("modalTitle").innerText = "Add Student Section";
...
// Edit
function editSS(f) {
    document.getElementById("modalTitle").innerText = "Update Student Section";
    document.getElementById("ss_id").value = f.id;
