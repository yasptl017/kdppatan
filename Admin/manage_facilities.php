...
.thumb-img{width:50px;height:50px;object-fit:cover;margin:2px;border-radius:4px;border:1px solid #ddd}
.photo-preview{display:flex;flex-wrap:wrap;gap:5px;justify-content:center}
.photo-selector{display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:15px;margin-top:15px}
.photo-item{border:3px solid #ddd;border-radius:8px;padding:8px;text-align:center;transition:all 0.3s;position:relative}
.photo-item.selected{border-color:#198754;background-color:#f0f9f4}
.photo-item img{width:100%;height:100px;object-fit:cover;border-radius:4px;display:block}
.photo-item input[type="checkbox"]{position:absolute;top:10px;right:10px;width:20px;height:20px;cursor:pointer;z-index:10}
.photo-filename{font-size:11px;margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#666}
.select-all-controls{display:flex;gap:10px;align-items:center;margin-bottom:10px;padding:10px;background:#f8f9fa;border-radius:6px}
</style>
                    ...
                    <td>
                        <div class="action-buttons">
                            ...
                            <button class="btn btn-warning btn-sm"
                                    onclick='editFacility(<?php echo json_encode($row, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>)'
                                    data-bs-toggle="modal"
                                    data-bs-target="#facilityModal"
                                    title="Edit">
...
function editFacility(facility) {
    ...
    let photos = [];
    try {
        if (typeof facility.photos === 'string') {
            photos = JSON.parse(facility.photos);
        } else if (Array.isArray(facility.photos)) {
