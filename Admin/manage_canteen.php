...
if (isset($_GET['move'], $_GET['id'])) {
    ...
    $current = $conn->query(
        "SELECT id, display_order FROM canteen WHERE id=$id"
    )->fetch_assoc();
    ...
    if ($current) {
        ...
        if ($direction === 'up') {
            $swap = $conn->query(
                "SELECT id, display_order FROM canteen
        ...
        if ($swap) {
            $conn->query(
                "UPDATE canteen SET display_order={$swap['display_order']}
                 WHERE id={$current['id']}"
            );
            $conn->query(
                "UPDATE canteen SET display_order={$current['display_order']}
                 WHERE id={$swap['id']}"
            );
        }
    }
...
/* =====================================
   ADD / UPDATE CANTEEN ENTRY
   ===================================== */
if (isset($_POST['save_canteen'])) {

    $id = intval($_POST['canteen_id']);
    ...
    $oldPhotos = [];
    if ($id > 0) {
        $q = $conn->query("SELECT photos FROM canteen WHERE id=$id LIMIT 1");
        if ($q->num_rows > 0) {
        ...
        foreach ($_FILES['photos']['name'] as $index => $fileName) {
            ...
            if ($_FILES['photos']['error'][$index] == 0) {
                ...
                if (in_array($ext, $allowed)) {
                    $newFileName = "canteen_" . time() . "_" . rand(1000, 9999) . "." . $ext;
                    $photoPath = $uploadDir . $newFileName;
                    
                    if (move_uploaded_file($_FILES['photos']['tmp_name'][$index], $photoPath)) {
                        $newPhotos[] = $photoPath;
                    }
                }
            }
    ...
    if ($id == 0) {
        $sql = "INSERT INTO canteen (title, display_order, photos, description)
                VALUES ('$title', $display_order, '$photosJSON', '$description')";
        $conn->query($sql);
        $message = "Canteen content added successfully!";
    } else {
        $sql = "UPDATE canteen SET 
                ...
                WHERE id=$id";
        $conn->query($sql);
        $message = "Canteen content updated successfully!";
    }
    $messageType = "success";
}
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM canteen WHERE id=$id");
    $message = "Canteen content deleted!";
    $messageType = "success";
}

// Fetch All
$records = $conn->query("SELECT * FROM canteen ORDER BY display_order ASC, id DESC");
?>
...
<main class="main-content" id="mainContent">
    ...
    <h2 class="h4 mb-4"><i class="fas fa-utensils me-2"></i>Manage Canteen</h2>
    ...
    <button class="btn btn-primary mb-3"
            ...
            data-bs-target="#canteenModal"
            onclick="addCanteen()">
        <i class="fas fa-plus-circle"></i> Add Canteen Content
    </button>

    <div class="form-card">
        <table id="canteenTable" class="table table-bordered table-striped align-middle">
            <thead>
                    ...
                    <td>
                        <div class="action-buttons">
                            ...
                            <button class="btn btn-warning btn-sm"
                                    onclick='editCanteen(<?php echo json_encode($row, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>)'
                                    data-bs-toggle="modal"
                                    data-bs-target="#canteenModal"
                                    title="Edit">
                            ...
                            <a href="?delete=<?php echo $row['id']; ?>"
                               onclick="return confirm('Delete this canteen item and all its photos?');"
...
<!-- MODAL -->
<div class="modal fade" id="canteenModal">
        ...
        <div class="modal-content">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="modalTitle">Add Canteen Content</h5>
                ...
                <div class="modal-footer">
                    <button type="submit" name="save_canteen" class="btn btn-success">
...
<script>
console.log('=== Canteen Management ===');
...
$('#canteenModal').on('shown.bs.modal', function () {
    if (!$('#description').next('.note-editor').length) {
        initSummernote('#description', {height: 250, placeholder: "Enter canteen description..."});
    }
    if (window.pendingDesc !== undefined) {
        $('#description').summernote('code', window.pendingDesc);
        window.pendingDesc = undefined;
    }
});

// Destroy Summernote when modal closes
$('#canteenModal').on('hidden.bs.modal', function () {
    if ($('#description').next('.note-editor').length) {
...
// Add Canteen
function addCanteen() {
    document.getElementById("modalTitle").innerText = "Add Canteen Content";
    document.getElementById("canteen_id").value = "";
...
// Edit Canteen
function editCanteen(f) {
    document.getElementById("modalTitle").innerText = "Update Canteen Content";
    document.getElementById("canteen_id").value = f.id;
    ...
    window.pendingDesc = f.description;
}
...
console.log('✓ Canteen Management page loaded');
</script>
